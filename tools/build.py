...
def render_dist(data: Dict[str, Any]) -> str:
    """Return the pretty-printed distribution JSON as a string.

    Pure function: no I/O, no timestamp, byte-identical for identical
    input.
    """
...
def build_dist(data: Dict[str, Any], output_path: Path) -> None:
    ...
    """
    _write_atomic(output_path, render_dist(data))
    print(f"Created {output_path}")

def render_minified(data: Dict[str, Any]) -> str:
    """Return the minified JSON as a string. Pure function."""
...
def render_csv(data: Dict[str, Any]) -> str:
    """Return the flat CSV as a string.

    Raises ValueError when an action cannot be flattened (nested dict,
    list value) or when there are no actions.
    """
    actions = data.get("actions", [])
    if not actions:
        raise ValueError("No actions to export to CSV")
...
def render_sql(data: Dict[str, Any]) -> str:
    """Return the SQL dump as a string.

    Raises ValueError when there are no actions.
    """
    actions = data.get("actions", [])
    if not actions:
        raise ValueError("No actions to export to SQL")
...
def main() -> int:
    ...
    parser.add_argument(
        "--check",
        action="store_true",
        help="Verify that committed artifacts match the current data. "
    ...
    # Phase 1: render every artifact in memory. Any failure — a
    # flatten_action ValueError, an unexpected key — leaves the output
    # directory untouched.
    try:
        payloads: Dict[Path, str] = {
            out_dir / f"{base}.dist.json": render_dist(data),
            out_dir / f"{base}.min.json":  render_minified(data),
            out_dir / f"{base}.csv":       render_csv(data),
            out_dir / f"{base}.sql":       render_sql(data),
        }
    except (OSError, ValueError) as e:
        print(f"Error: {e}", file=sys.stderr)
        return 2

    # --check: compare in-memory payloads against the committed files.
    # The .meta.json sidecar is deliberately excluded: it carries a
    # timestamp and would differ on every run.
    if args.check:
        drift: List[str] = []
        ...
        if drift:
            ...
            print(
                "\nRegenerate with:\n"
                f"  python3 tools/build.py --actions {args.actions} "
                f"--output-dir {out_dir}",
                file=sys.stderr,
            )
            return 1
        print("OK: all artifacts match.")
        return 0

    # Phase 2: write every artifact. Two-phase commit means partial
    # failure cannot leave a mix of fresh and stale files on disk.
    try:
